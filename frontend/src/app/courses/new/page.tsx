...
export default function NewCoursePage() {
  ...
  const [data, setData] = useState<CreateCourseDTO>({
    code: "",
    name: "",
    credits: 0,
    maxCapacity: 0,
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([subjRes, teacherRes, classroomRes, periodRes]) => {
  ...
  const handleDaysChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setData((prev) => ({
      ...prev,
      schedule: { ...prev.schedule, days: e.target.value.split(",").map((d) => d.trim()).filter(Boolean) },
    }))
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              {errors["schedule.days"] && (
                <p className="mt-1 text-xs text-red-500">{errors["schedule.days"]}</p>
              )}
            </div>
